...
function editTask() {
	return displayUtils.getItemUpdateInputHandler(currentTask, {
		steps: ['Product', 'Target Quantity'],
		getters: {
			"Product": 'product',
			"Target Quantity": 'targetQuantity'
		},
		setters: {
			"Product": function(task, value) {
				task.product = value;
			},
			"Target Quantity": function(task, value) {
				task.targetQuantity = parseFloat(value);
			}
		}
	});
}
